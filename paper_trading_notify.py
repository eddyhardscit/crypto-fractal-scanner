...
def build_event_messages(summary: dict[str, Any]) -> list[str]:
    ...
    if not opened and not closed and not trailing and not risk_alerts and not risk_recoveries:
        return []

    lines = ["🧪 PAPER TRADING KUCOIN — EVENTI"]
    for position in opened:
        icon = "🟢" if position.get("side") == "LONG" else "🔴"
        lines.extend(
            [
                "",
                f"{icon} APERTURA {position.get('portfolio', '')}",
                f"{position.get('asset', '')} {position.get('side', '')} · "
                f"{float(position.get('leverage', 0.0)):.1f}x · TF {position.get('timeframe_minutes', '')}m",
                f"Entry {float(position.get('entry_price', 0.0)):.6g} · "
                f"Stop {float(position.get('stop_price', 0.0)):.6g} · "
                f"Target {float(position.get('target_price', 0.0)):.6g}",
                f"Margine {_fmt_eur(position.get('margin_eur'))} · "
                f"Rischio {_fmt_eur(position.get('initial_risk_eur'))}",
            ]
        )
    ...
    for update in trailing:
        lines.extend(
            [
                "",
                f"🔄 STOP AGGIORNATO {update.get('portfolio', '')}",
                f"{update.get('asset', '')} {update.get('side', '')} · "
    ...
    for trade in closed:
        ...
        lines.extend(
            [
                "",
                f"{icon} CHIUSURA {trade.get('portfolio', '')}",
                f"{trade.get('asset', '')} {trade.get('side', '')} · {trade.get('close_reason', '')}",
                f"P&L netto {_fmt_eur(pnl, signed=True)} · R {_fmt_num(trade.get('r_multiple'))}",
    ...
    for alert in risk_alerts:
        lines.extend(
            [
                "",
                f"⚠️ BLOCCO RISCHIO {alert.get('portfolio', '')}",
    ...
    for recovery in risk_recoveries:
        lines.extend(
            [
                "",
                f"✅ RISCHIO RIABILITATO {recovery.get('portfolio', '')}",
...
def build_status_message(state: dict[str, Any], config: dict[str, Any]) -> str:
    ...
    main = next((row for row in metrics if row.get("is_main")), metrics[0])
    initial = float(state.get("initial_capital_eur", config.get("initial_capital_eur", 0.0)))
    ...
    lines = [
        ...
        "",
        f"MAIN: equity {_fmt_eur(equity)} · rendimento {_fmt_pct(total_return, signed=True)}",
        f"P&L mese {_fmt_eur(main.get('month_pnl_eur'), signed=True)} / "
        f"target {_fmt_eur(main.get('monthly_target_eur'))}",
        f"Aperte {main.get('open_positions', 0)} · Chiuse {main.get('closed_trades', 0)} · "
    ...
    for row in metrics:
        if row.get("is_main"):
            continue
        lines.append(
            f"• {row.get('portfolio')}: {_fmt_eur(row.get('equity_eur'))} · "
            f"trade {row.get('closed_trades', 0)} · aperte {row.get('open_positions', 0)} · "
    ...
    for portfolio_name, portfolio in state.get("portfolios", {}).items():
        for position in portfolio.get("open_positions", []):
            open_lines.append(
                f"• {portfolio_name}: {position.get('asset')} {position.get('side')} · "
                f"entry {float(position.get('entry_price', 0.0)):.6g} · "
                f"stop {float(position.get('stop_price', 0.0)):.6g} · "
                f"target {float(position.get('target_price', 0.0)):.6g}"
            )
    lines.extend(["", "Posizioni aperte:"])
    lines.extend(open_lines[:12] if open_lines else ["• Nessuna posizione virtuale aperta"])
