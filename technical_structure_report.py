...
# Fibonacci automatico: usa swing pivot oggettivi e pesa al massimo ±1.
FIB_RATIOS = (0.236, 0.382, 0.500, 0.618, 0.786)
FIB_EXTENSIONS = (1.272, 1.618)
FIB_LOOKBACK_DAYS = 140
FIB_MIN_SWING_PCT = {"BTC": 7.0, "SOL": 10.0, "DOGE": 12.0}
FIB_MIN_SWING_ATR = 2.5
FIB_TOUCH_ATR = 0.50
FIB_NEAR_ATR = 1.00
FIB_CONFLUENCE_ATR = 0.75
...
def distance_to_neckline(direction, close, neckline):
    """Direction-aware remaining distance to a candidate breakout."""
    close = safe_float(close)
    neckline = safe_float(neckline)
...
def evaluate_pattern_lifecycle(
    ...
    if breakout_date is None:
        result.update({
            ...
            "confirmed": False,
            "score": 0,
            # Il target non è ancora attivo: mostrare un progresso negativo è fuorviante.
            "target_progress_pct": np.nan,
            "distance_to_neckline_pct": distance_to_neckline(direction, close, neckline),
            "details": (
                f"{geometry_details} Stato: CANDIDATO; la neckline non è ancora stata "
                f"rotta con un margine di almeno {BREAKOUT_BUFFER_PCT:.2f}%. "
                f"Età della formazione: {formation_age} giorni."
            ),
        })
        return result
...
def pattern_adam_eve_bottom(df, ticker):
    ...
    details = (
        ...
        f"{d1.date()} al {d2.date()}. Un minimo è più appuntito e l'altro più "
...
def pattern_adam_eve_top(df, ticker):
    ...
    details = (
        ...
        f"{d1.date()} al {d2.date()}. Un massimo è più appuntito e l'altro più "
...
def _fib_pivot_points(df):
    recent = recent_slice(df, FIB_LOOKBACK_DAYS)
    points = []
    for date, row in recent.iterrows():
        if bool(row.get("pivot_low", False)):
            points.append({"date": pd.Timestamp(date), "kind": "LOW", "price": safe_float(row.get("Low"))})
        if bool(row.get("pivot_high", False)):
            points.append({"date": pd.Timestamp(date), "kind": "HIGH", "price": safe_float(row.get("High"))})
    points = [p for p in points if not pd.isna(p["price"])]
    return sorted(points, key=lambda p: p["date"])


def select_fibonacci_impulse(df, ticker):
    """Select one recent objective swing from confirmed pivot points."""
    points = _fib_pivot_points(df)
    if len(points) < 2:
        return {}

    asset = ticker.replace("-USD", "")
    last_date = pd.Timestamp(df.index[-1])
    current_atr = safe_float(df["atr14"].iloc[-1])
    min_pct = FIB_MIN_SWING_PCT.get(asset, 10.0)
    candidates = []

    # Use all opposite pivot pairs, but strongly prefer recent completed swings.
    for i, start in enumerate(points[:-1]):
        for end in points[i + 1 :]:
            if start["kind"] == end["kind"]:
                continue
            duration = max(1, (end["date"] - start["date"]).days)
            endpoint_age = max(0, (last_date - end["date"]).days)
            if duration < 5 or duration > 120 or endpoint_age > 60:
                continue

            amplitude = abs(end["price"] - start["price"])
            base = min(start["price"], end["price"])
            amplitude_pct = amplitude / base * 100 if base > 0 else np.nan
            amplitude_atr = amplitude / current_atr if current_atr and current_atr > 0 else np.nan

            if pd.isna(amplitude_pct) or amplitude_pct < min_pct:
                continue
            if not pd.isna(amplitude_atr) and amplitude_atr < FIB_MIN_SWING_ATR:
                continue
            ...
            # Recency dominates; amplitude and clean duration are secondary.
            score = 180.0 - endpoint_age * 3.0 + min(amplitude_pct, 80.0) * 0.55 - duration * 0.08
            candidates.append(
                {
                    "direction": direction,
                    "start_date": safe_date_str(start["date"]),
                    "end_date": safe_date_str(end["date"]),
                    ...
                    "endpoint_age_days": endpoint_age,
...
def fibonacci_levels(impulse):
    if not impulse:
        return {}, {}
    low = safe_float(impulse.get("low"))
    high = safe_float(impulse.get("high"))
    direction = impulse.get("direction")
    if pd.isna(low) or pd.isna(high) or high <= low:
        return {}, {}
    width = high - low
    if direction == "UP":
        retracements = {ratio: high - width * ratio for ratio in FIB_RATIOS}
        extensions = {ratio: low + width * ratio for ratio in FIB_EXTENSIONS}
    else:
        retracements = {ratio: low + width * ratio for ratio in FIB_RATIOS}
        extensions = {ratio: high - width * ratio for ratio in FIB_EXTENSIONS}
    return retracements, extensions


def fibonacci_level_state(df, direction, level):
    level = safe_float(level)
    if pd.isna(level):
        return "NON_ATTIVO", np.nan, np.nan

    recent = df.tail(6)
    close = safe_float(recent["Close"].iloc[-1])
    atr = safe_float(recent["atr14"].iloc[-1])
    if pd.isna(close):
        return "NON_ATTIVO", np.nan, np.nan

    tolerance = max((atr or 0.0) * FIB_TOUCH_ATR, abs(close) * 0.003)
    near_distance = max((atr or 0.0) * FIB_NEAR_ATR, abs(close) * 0.01)
    ...
    state = "NON_ATTIVO"
    if direction == "UP":
        if len(closes) >= 4 and (closes.iloc[-4:-2] < level - tolerance).all() and (closes.iloc[-2:] > level).all():
            state = "RECUPERATO"
        elif len(closes) >= 2 and (closes.iloc[-2:] < level - tolerance).all():
            state = "PERSO"
        elif len(lows) >= 3 and lows.iloc[-3:].min() <= level + tolerance and len(closes) >= 2 and (closes.iloc[-2:] >= level).all():
            state = "TENUTO"
        elif highs.iloc[-1] >= level - tolerance and lows.iloc[-1] <= level + tolerance:
            state = "TESTATO"
        elif abs(close - level) <= near_distance:
            state = "IN_AVVICINAMENTO"
    else:
        if len(closes) >= 2 and (closes.iloc[-2:] > level + tolerance).all():
            state = "RECUPERATO"
        elif len(highs) >= 3 and highs.iloc[-3:].max() >= level - tolerance and len(closes) >= 2 and (closes.iloc[-2:] <= level).all():
            state = "REJECTION"
        elif highs.iloc[-1] >= level - tolerance and lows.iloc[-1] <= level + tolerance:
            state = "TESTATO"
        elif abs(close - level) <= near_distance:
    ...
    distance_pct = (close / level - 1.0) * 100.0 if level else np.nan
    distance_atr = abs(close - level) / atr if atr and atr > 0 else np.nan
    return state, distance_pct, distance_atr


def fibonacci_analysis(df, ticker, independent_levels):
    impulse = select_fibonacci_impulse(df, ticker)
    retracements, extensions = fibonacci_levels(impulse)
    if not impulse or not retracements:
        return {
            "fib_score": 0,
            "fib_state": "NON_ATTIVO",
            "fib_role": "n/a",
            "fib_direction": "n/a",
            "fib_details": "Nessuno swing pivot recente abbastanza ampio e pulito.",
        }

    close = safe_float(df["Close"].iloc[-1])
    atr = safe_float(df["atr14"].iloc[-1])
    ratio, level = min(retracements.items(), key=lambda item: abs(close - item[1]))
    state, distance_pct, distance_atr = fibonacci_level_state(df, impulse["direction"], level)
    confluence_tolerance = max((atr or 0.0) * FIB_CONFLUENCE_ATR, abs(close) * 0.0075)

    confluences = []
    for label, value in independent_levels.items():
        value = safe_float(value)
        if pd.isna(value):
            continue
        if abs(value - level) <= confluence_tolerance:
            confluences.append(label)

    score = 0
    if confluences:
        if impulse["direction"] == "UP":
            if state in {"TENUTO", "RECUPERATO"}:
                score = 1
            elif state == "PERSO":
                score = -1
        else:
            if state == "REJECTION":
                score = -1
            elif state == "RECUPERATO":
                score = 1

    role = "SUPPORTO DI RITRACCIAMENTO" if impulse["direction"] == "UP" else "RESISTENZA DI RIMBALZO"
    confl_text = ", ".join(confluences) if confluences else "nessuna confluenza indipendente"
    details = (
        f"Swing {impulse['direction']} {impulse['start_date']} {fmt_price(ticker, impulse['start_price'])} -> "
        f"{impulse['end_date']} {fmt_price(ticker, impulse['end_price'])}; livello più vicino "
        f"{ratio * 100:.1f}% a {fmt_price(ticker, level)}; stato {state.replace('_', ' ')}; "
        f"confluenza: {confl_text}."
    )
    ...
    out = {
        ...
        "fib_state": state,
        "fib_role": role,
        "fib_direction": impulse["direction"],
        "fib_start_date": impulse["start_date"],
        "fib_end_date": impulse["end_date"],
        "fib_start_price": impulse["start_price"],
        "fib_end_price": impulse["end_price"],
        "fib_amplitude_pct": impulse["amplitude_pct"],
        "fib_nearest_ratio": ratio,
        "fib_nearest_level": level,
        "fib_distance_pct": distance_pct,
        "fib_distance_atr": distance_atr,
        "fib_confluence": confl_text,
        "fib_details": details,
    }
    for fib_ratio, fib_level in retracements.items():
        out[f"fib_{int(round(fib_ratio * 1000)):03d}"] = fib_level
    for fib_ratio, fib_level in extensions.items():
        out[f"fib_ext_{int(round(fib_ratio * 1000)):04d}"] = fib_level
    return out
...
def add_pattern_columns(row, prefix, result):
    ...
    row[f"{prefix}_distance_to_neckline_pct"] = result.get("distance_to_neckline_pct", np.nan)
...
def analyze_asset(ticker):
    ...
    raw = apply_snapshot_to_ohlcv(raw, ticker)
    ...
    fib = fibonacci_analysis(
        ...
        ticker,
        {
            ...
            "resistenza tecnica": resistance,
            "neckline rialzista": dominant_bullish.get("neckline"),
            "invalidazione rialzista": dominant_bullish.get("invalidation_level"),
            "neckline ribassista": dominant_bearish.get("neckline"),
            "invalidazione ribassista": dominant_bearish.get("invalidation_level"),
        },
    )
    fib_score = safe_int(fib.get("fib_score"), 0)
    ...
    # Compatibilità con i nomi già letti da altri script.
...
def pattern_cell(status, variant=""):
    ...
    if variant:
        return f"{it_label(variant)} — {status_txt}"
    return status_txt
...
def append_pattern_detail(lines, ticker, label, row, prefix, variant_col=None):
    ...
    if status != "ASSENTE":
        ...
        distance = row.get(f"{prefix}_distance_to_neckline_pct")
        progress_text = (
            f"distanza dalla neckline {fmt_pct(distance)}"
            if status == "CANDIDATO"
            else f"progresso {fmt_pct(progress)}"
        )
...
def render_report(metrics):
    ...
    lines.append("- Data breakout, età, target teorico, progresso e recupero della neckline")
    ...
    lines.append("- Fibonacci automatico su swing pivot, con lifecycle e confluenza")
    ...
    for _, r in metrics.iterrows():
        ...
        summary_rows.append({
            ...
            "Fibonacci": f"{fmt_signed_int(r.get('fib_score', 0))} / {str(r.get('fib_state', 'NON_ATTIVO')).replace('_', ' ')}",
            "Pattern rialzista": (
    ...
    for _, r in metrics.iterrows():
        ...
        lines.append(f"- Fibonacci automatico: **{str(r.get('fib_state', 'NON_ATTIVO')).replace('_', ' ')}** ({fmt_signed_int(r.get('fib_score', 0))})")
        lines.append(f"  - {r.get('fib_details', 'n/a')}")
    ...
    lines.append("## Fibonacci automatico")
    lines.append("")
    lines.append(
        "Il modulo seleziona uno swing recente tramite pivot confermati. Un semplice tocco vale 0: "
        "Fibonacci pesa al massimo ±1 soltanto quando il livello è tenuto, perso, recuperato o respinto "
        "e coincide con almeno un livello tecnico indipendente."
    )
    lines.append("")
    fib_rows = []
    for _, r in metrics.iterrows():
        ticker = r["ticker"]
        nearest_ratio = safe_float(r.get("fib_nearest_ratio"))
        nearest_label = "n/a" if pd.isna(nearest_ratio) else f"{nearest_ratio * 100:.1f}%"
        fib_rows.append({
            "Asset": r["asset"],
            "Swing": f"{r.get('fib_direction', 'n/a')} {r.get('fib_start_date', '')} -> {r.get('fib_end_date', '')}",
            "23,6%": fmt_price(ticker, r.get("fib_236")),
            "38,2%": fmt_price(ticker, r.get("fib_382")),
            "50,0%": fmt_price(ticker, r.get("fib_500")),
            "61,8%": fmt_price(ticker, r.get("fib_618")),
            "78,6%": fmt_price(ticker, r.get("fib_786")),
            "Livello vicino": f"{nearest_label} / {fmt_price(ticker, r.get('fib_nearest_level'))}",
            "Stato": str(r.get("fib_state", "NON_ATTIVO")).replace("_", " "),
            "Confluenza": r.get("fib_confluence", "n/a"),
            "Score": fmt_signed_int(r.get("fib_score", 0)),
        })
    lines.append(df_to_markdown(pd.DataFrame(fib_rows)))
    lines.append("")

    lines.append("## Stati del ciclo di vita")
    lines.append("")
    lines.append("- **CANDIDATO**: geometria presente, ma neckline non ancora rotta; punteggio 0.")
    lines.append("- **ATTIVO**: breakout avvenuto da 0 a 3 giorni; peso prudente ±1.")
    lines.append("- **CONFERMATO RECENTE**: breakout da 4 a 14 giorni; peso massimo prudente ±2.")
    lines.append("- **MATURO**: breakout più vecchio di 14 giorni e ancora valido; peso ridotto ±1.")
    lines.append("- **TARGET RAGGIUNTO**: movimento teorico già sviluppato; punteggio 0.")
    ...
    lines.append(
        ...
        "la stessa struttura non vengono più sommati tutti insieme."
    )
    lines.append("")
    ...
    lines.append("")
    lines.append(
        "Nota importante: questo report non è una previsione da solo. È un filtro tecnico "
