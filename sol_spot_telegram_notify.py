...
def _status_icon(action: str) -> str:
    action = action.upper()
...
def _message(state: dict[str, Any]) -> str:
    ...
    equity = float(state.get("equity_eur", 40000.0))
    cash = float(state.get("cash_eur", 40000.0))
    sol_qty = float(state.get("sol_qty", 0.0))
    return_pct = float(state.get("return_pct", 0.0))
    ...
    return (
        "🟣 SOL SPOT ADAPTIVE\n"
        "Paper trading separato\n\n"
        f"{_status_icon(action)} Stato: {action}\n"
        f"Motivo: {reason}\n\n"
        "💰 Portafoglio\n"
        f"Equity: {_fmt_eur(equity)} ({return_pct:+.2f}%)\n"
        f"Cash: {_fmt_eur(cash)}\n"
        f"SOL: {sol_qty:.6f}\n"
        f"Prezzo: {_fmt_price(price)}\n\n"
        "🎯 Bande operative 4H\n"
        f"L2  {float(bands.get('lower2', 0.0)):.2f}\n"
        f"L1  {float(bands.get('lower1', 0.0)):.2f}\n"
        f"MID {float(bands.get('mid', 0.0)):.2f}\n"
        f"U1  {float(bands.get('upper1', 0.0)):.2f}\n"
        f"U2  {float(bands.get('upper2', 0.0)):.2f}\n\n"
        "📊 Statistiche\n"
        f"Operazioni: {trades}\n"
        f"Max drawdown: {max_dd:.2f}%\n\n"
        "Solo spot · nessuna leva · capitale €40.000"
    )
...
def _send_document(token: str, chat_id: str) -> None:
    ...
    with REPORT_PATH.open("rb") as handle:
        response = requests.post(
            ...
            data={
                "chat_id": chat_id,
                "caption": "Report completo SOL Spot Adaptive.",
            },
