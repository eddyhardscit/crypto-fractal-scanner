# -*- coding: utf-8 -*-
"""Offline self-test for Exchange Microstructure Upgrade v2.1.1.
...
def test_geo_short_circuit() -> None:
    import exchange_market_data as emd

    original_get_json = emd.get_json
    calls: list[tuple[str, str]] = []

    def blocked_binance(session, base, path, params):
        calls.append((base, path))
        return emd.FetchResult(
            False,
            error="HTTP 451: Service unavailable from a restricted location according to Eligibility",
            url=base + path,
            status_code=451,
        )

    emd.get_json = blocked_binance
    try:
        payload, health = emd.fetch_binance_asset(object(), "BTC")
    finally:
        emd.get_json = original_get_json
    if len(calls) != 1:
        raise AssertionError(f"Binance should stop after first geo block, calls={calls}")
    if payload["parsed"].get("source_state") != "GEO_BLOCKED":
        raise AssertionError(f"Binance geo state missing: {payload}")
    if not all(row.get("state") == "GEO_BLOCKED" for row in health.values()):
        raise AssertionError(f"Binance skipped endpoints not classified: {health}")

    calls.clear()

    def blocked_bybit(session, base, path, params):
        calls.append((base, path))
        return emd.FetchResult(
            False,
            error="HTTP 403: The Amazon CloudFront distribution is configured to block access from your country",
            url=base + path,
            status_code=403,
        )

    emd.get_json = blocked_bybit
    try:
        payload, health = emd.fetch_bybit_asset(object(), "SOL")
    finally:
        emd.get_json = original_get_json
    if len(calls) != 1:
        raise AssertionError(f"Bybit should stop after first geo block, calls={calls}")
    if payload["parsed"].get("source_state") != "GEO_BLOCKED":
        raise AssertionError(f"Bybit geo state missing: {payload}")
    if not all(row.get("state") == "GEO_BLOCKED" for row in health.values()):
        raise AssertionError(f"Bybit skipped endpoints not classified: {health}")
...
def test_report_and_tracker() -> None:
    with tempfile.TemporaryDirectory(prefix="exchange-v2-report-") as tmp_name:
        ...
        # v2.1.1 saves the first daily exchange snapshot before running the tracker,
        # then refreshes the report with updated tracker metrics.
        ...
        for token in ("Binance", "Bybit", "KuCoin", "LOCKED / RACCOLTA 7G", "Taker flow (campione/4h)"):
            if token not in report:
                raise AssertionError(f"Report token missing: {token}")
        diagnostics = reports / "exchange_source_diagnostics.md"
        if not diagnostics.exists() or "Diagnostica fonti exchange" not in diagnostics.read_text(encoding="utf-8"):
            raise AssertionError("Committed source diagnostics was not generated")
        tracker_report = (reports / "exchange_signal_tracker_report.md").read_text(encoding="utf-8")
        if "Nessuna fotografia exchange ancora salvata" in tracker_report:
            raise AssertionError("Tracker ran before first daily snapshot")
...
def test_persistent_storage() -> None:
    with tempfile.TemporaryDirectory(prefix="exchange-v2-1-storage-") as tmp_name:
        ...
        (reports / "exchange_market_data_snapshot.json").write_text(
            json.dumps({"schema_version": "2.1.1", "assets": {}}), encoding="utf-8"
        )
        (reports / "exchange_market_data_health.json").write_text(
            json.dumps({"schema_version": "2.1.1", "status": "OK"}), encoding="utf-8"
        )
...
def test_installer() -> None:
    ...
    if missing:
        ...
        return
    tracker_source = (source_root / "module_signal_tracker.py").read_text(encoding="utf-8")
    if '"key": "exchange_microstructure"' not in tracker_source:
        print("Installer integration test skipped; base scanner is not yet on v2.1.")
        return
    ...
    with tempfile.TemporaryDirectory(prefix="exchange-v2-install-") as tmp_name:
        ...
        for name in (
            ...
            "exchange_intraday_workflow.yml",
            "apply_exchange_microstructure_upgrade_v2_1_1.py",
        ):
            shutil.copy2(ROOT / name, tmp / name)
        ...
        subprocess.run([sys.executable, "apply_exchange_microstructure_upgrade_v2_1_1.py"], cwd=tmp, check=True)
        ...
        first_report = daily_text.find("python exchange_microstructure_report.py")
        tracker_pos = daily_text.find("python exchange_signal_tracker.py")
        second_report = daily_text.find("python exchange_microstructure_report.py", first_report + 1)
        global_pos = daily_text.find("python global_confluence_report.py")
        positions = [
            daily_text.find("python exchange_persistent_storage.py restore"),
            daily_text.find("python exchange_persistent_storage.py audit"),
            daily_text.find("python exchange_persistent_storage.py archive-completed-months"),
            first_report,
            tracker_pos,
            second_report,
            global_pos,
        ...
        if 'lines.append("# EXCHANGE_MICROSTRUCTURE_TRACKER_PATCH' in tracker_text:
            raise AssertionError("Visible installer marker was not removed")


def main() -> None:
    test_kucoin_parser()
    test_geo_short_circuit()
    test_report_and_tracker()
    test_persistent_storage()
    test_installer()
    print("Exchange Microstructure Upgrade v2.1.1 self-test: OK")
