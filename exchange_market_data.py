...
def build_session() -> requests.Session:
    ...
    session.headers.update(
        {
            "User-Agent": "crypto-scanner-exchange-microstructure/2.1",
...
def fetch_binance_asset(session: requests.Session, asset: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    calls = {
        "premium": get_json(session, BINANCE_BASE, "/fapi/v1/premiumIndex", {"symbol": symbol}),
...
def fetch_bybit_asset(session: requests.Session, asset: str) -> tuple[dict[str, Any], dict[str, Any]]:
    symbol = SYMBOLS[asset]
    raw_calls = {
        "ticker": get_json(session, BYBIT_BASE, "/v5/market/tickers", {"category": "linear", "symbol": symbol}),
        "open_interest_hist": get_json(
            session,
            BYBIT_BASE,
...
def collect_binance_liquidations(
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            received += 1
        ws.close()
        health["binance"] = {"ok": True, "messages": received, "error": ""}
    except Exception as exc:
        health["binance"] = {"ok": False, "messages": 0, "error": f"{type(exc).__name__}: {exc}"}
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            for row in rows:
                ...
                received += 1
        ws.close()
        health["bybit"] = {"ok": True, "messages": received, "error": ""}
    except Exception as exc:
        health["bybit"] = {"ok": False, "messages": 0, "error": f"{type(exc).__name__}: {exc}"}


def collect_liquidation_sample() -> tuple[dict[str, Any], dict[str, Any]]:
    totals = liquidation_empty()
    health: dict[str, Any] = {
        "sample_seconds": LIQ_SAMPLE_SECONDS,
        "skipped": SKIP_WEBSOCKET or LIQ_SAMPLE_SECONDS <= 0,
        "binance": {"ok": False, "messages": 0, "error": "not run"},
        "bybit": {"ok": False, "messages": 0, "error": "not run"},
    }
    if health["skipped"]:
    ...
    lock = threading.Lock()
    threads = [
        threading.Thread(target=collect_binance_liquidations, args=(totals, lock, deadline, health), daemon=True),
        threading.Thread(target=collect_bybit_liquidations, args=(totals, lock, deadline, health), daemon=True),
    ]
...
def main() -> None:
    ...
    for asset in ASSETS:
        binance, binance_health = fetch_binance_asset(session, asset)
        bybit, bybit_health = fetch_bybit_asset(session, asset)
        kucoin, kucoin_health = fetch_kucoin_asset(session, asset)
    ...
    liquidations, liquidation_health = collect_liquidation_sample()
