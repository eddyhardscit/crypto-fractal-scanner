...
def build_operating_plan(
    ...
    # Indicative profit-taking map based on the current adaptive range.
    # These are not pending orders and do not alter the live strategy.
    half_width = max(0.0, upper - center)
    sale_prices = [
        max(center, break_even or center),
        max(upper, (break_even or 0.0) * 1.01),
        max(upper + half_width, (break_even or 0.0) * 1.03),
    ]
    sale_fractions = [0.25, 0.35, 0.40]
    sale_levels: list[dict[str, Any]] = []
    total_sale_net = 0.0
    total_sale_cost = 0.0

    for index, (sale_price, fraction) in enumerate(
        zip(sale_prices, sale_fractions),
        start=1,
    ):
        sale_quantity = sol_units * fraction
        gross = sale_price * sale_quantity
        net = gross * (1.0 - fee_rate - slippage_rate)
        cost = average_cost * sale_quantity
        pnl = net - cost
        total_sale_net += net
        total_sale_cost += cost
        sale_levels.append(
            {
                "level": index,
                "price_eur": sale_price,
                "quantity_sol": sale_quantity,
                "fraction_of_position_pct": fraction * 100.0,
                "estimated_net_proceeds_eur": net,
                "estimated_pnl_eur": pnl,
            }
        )

    estimated_cycle_pnl = total_sale_net - total_sale_cost
    estimated_cycle_roi = (
        estimated_cycle_pnl / total_sale_cost * 100.0
        if total_sale_cost > 0
        else 0.0
    )
    ...
    return {
        ...
        "used_tranches_estimate": (
            max(1, int(round((average_cost * sol_units) / trade_limit)))
            if trade_limit > 0 and sol_units > 0
            else 0
        ),
        "total_tranches_estimate": (
            max(
                1,
                int(math.ceil(
                    max(0.0, equity - reserve) / trade_limit
                )),
            )
            if trade_limit > 0
            else 0
        ...
        "sale_plan": {
            ...
            "levels": sale_levels,
            "estimated_total_net_proceeds_eur": total_sale_net,
            "estimated_total_pnl_eur": estimated_cycle_pnl,
            "estimated_roi_pct": estimated_cycle_roi,
            "note": (
                "Tre livelli indicativi derivati dal range corrente. "
                "Il bot non piazza take-profit fissi: ricalcola peso obiettivo "
                "e quantità a ogni candela."
            ),
        },
        "defensive_plan": {
...
def build_metrics() -> dict[str, Any]:
    ...
    if not trades.empty:
        sides = trades.get("side", pd.Series(dtype=str)).astype(str).str.upper()
        buy_count = int((sides == "BUY").sum())
        sell_count = int((sides == "SELL").sum())
        if "time_utc" in trades.columns:
            trade_times = pd.to_datetime(
                trades["time_utc"], utc=True, errors="coerce"
            ).dropna()
            if not trade_times.empty:
                cycle_start = trade_times.min()
                cycle_start_utc = cycle_start.isoformat()
                cycle_duration_days = (
                    datetime.now(timezone.utc) - cycle_start.to_pydatetime()
    ...
    return {
        ...
        "cycle": {
            ...
            "realized_pnl_eur": safe_float(
                state.get("realized_pnl_eur"), 0.0
            ),
        },
...
def render_markdown(metrics: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        "| Voce | Valore |",
        "| --- | ---: |",
        f"| Esposizione reale | {fmt_num(plan['current_weight_pct'])}% |",
        f"| Esposizione ideale calcolata | {fmt_num(plan['target_weight_pct'])}% |",
        f"| Differenza | {fmt_num(plan['target_weight_pct'] - plan['current_weight_pct'])}% |",
        f"| Tranche utilizzate stimate | {plan['used_tranches_estimate']} / {plan['total_tranches_estimate']} |",
        f"| Tranche residue stimate | {plan['estimated_remaining_tranches']} |",
        "",
        ...
        "| Voce | Valore |",
        "| --- | ---: |",
        f"| Inizio ciclo | {metrics['cycle']['start_utc'] or 'N/D'} |",
        f"| Durata | {fmt_num(metrics['cycle']['duration_days'], 1)} giorni |",
        f"| Acquisti eseguiti | {metrics['cycle']['buy_count']} |",
        f"| Vendite eseguite | {metrics['cycle']['sell_count']} |",
        f"| P/L realizzato ciclo | {fmt_eur(metrics['cycle']['realized_pnl_eur'])} |",
        "",
        ...
        "| ---: | ---: | ---: | ---: | ---: | ---: |",
        *[
            f"| {level['level']} | {fmt_eur(level['price_eur'])} | "
            f"{fmt_num(level['quantity_sol'], 6)} SOL | "
            f"{fmt_num(level['fraction_of_position_pct'])}% | "
            f"{fmt_eur(level['estimated_net_proceeds_eur'])} | "
            f"{fmt_eur(level['estimated_pnl_eur'])} |"
            for level in plan['sale_plan']['levels']
        ],
        "",
        f"| Profitto netto teorico complessivo | {fmt_eur(plan['sale_plan']['estimated_total_pnl_eur'])} |",
        f"| ROI teorico della posizione | {fmt_pct(plan['sale_plan']['estimated_roi_pct'])} |",
        "",
...
def telegram_text(metrics: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        "",
        "🧠 <b>Stato algoritmo e tranche</b>",
        f"Esposizione reale: <b>{fmt_num(plan['current_weight_pct'])}%</b>",
        f"Esposizione ideale: <b>{fmt_num(plan['target_weight_pct'])}%</b>",
        f"Differenza: <b>{fmt_num(plan['target_weight_pct'] - plan['current_weight_pct'])}%</b>",
        f"Tranche utilizzate: <b>{plan['used_tranches_estimate']} / {plan['total_tranches_estimate']}</b>",
        f"Tranche residue stimate: <b>{plan['estimated_remaining_tranches']}</b>",
        "",
        "📊 <b>Ciclo operativo corrente</b>",
        f"Inizio: <b>{metrics['cycle']['start_utc'] or 'N/D'}</b>",
        f"Durata: <b>{fmt_num(metrics['cycle']['duration_days'], 1)} giorni</b>",
        f"Acquisti: <b>{metrics['cycle']['buy_count']}</b>",
        f"Vendite: <b>{metrics['cycle']['sell_count']}</b>",
        f"P/L realizzato ciclo: <b>{fmt_eur(metrics['cycle']['realized_pnl_eur'])}</b>",
        "",
        "📋 <b>Piano operativo</b>",
        ...
        f"Motivo: {plan['reason']}",
        "",
        "💰 <b>Piano di vendita indicativo</b>",
        *[
            f"Vendita {level['level']}: <b>{fmt_eur(level['price_eur'])}</b> — "
            f"{fmt_num(level['quantity_sol'], 6)} SOL "
            f"({fmt_num(level['fraction_of_position_pct'])}%) — "
            f"P/L stimato {fmt_eur(level['estimated_pnl_eur'])}"
            for level in plan['sale_plan']['levels']
        ],
        f"Profitto netto teorico totale: <b>{fmt_eur(plan['sale_plan']['estimated_total_pnl_eur'])}</b>",
        f"ROI teorico posizione: <b>{fmt_pct(plan['sale_plan']['estimated_roi_pct'])}</b>",
        "<i>Livelli non piazzati: vengono ricalcolati con il range.</i>",
        "",
